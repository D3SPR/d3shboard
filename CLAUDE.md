...
### UI (`src/ui/`)
- `kit.tsx` holds shared primitives (`Field` with optional `help` tooltip, `Section`, `Disclosure`, `Segmented`, `Toggle`, `Slider`, `ColorField` with alpha, `Dialog`, `Popover`). `Slider` always pairs the track with a number box you can type into, and takes a plain `unit` suffix rather than a formatter — if a value needs converting for display (percentages, say), convert it at the call site so the number in the box is the number you'd type.
- **Adding anything happens in one place.** The toolbar's Add button opens `AddDialog`, whose two tabs are the component library (`LibraryPanel`, including a "Basics" section for the raw widget types) and the data sources (`DataPanel`). Both panels are plain content — the dialog owns the chrome — so keep them free of their own `Dialog` wrappers. The dialog hides itself while a basic is dragged onto the canvas. Popovers and tooltips portal to `document.body` and position from the anchor rect, so theme colours come from CSS variables (`--accent`, `--accent-ink`, `--chrome`) set on `document.documentElement` in `App.tsx`.
