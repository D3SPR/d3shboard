...
- **Timers** (stopwatch, countdown, focus/pomodoro, alarm) are one `timer` node with a `mode`. Their state lives in the component's params as timestamps — `run` (running since, 0 when stopped) and `acc` (time banked) — so a running timer keeps counting through a reload and elapsed time is always `acc + (now − run)`. Changes that move time between the two go through `patchParams`, which updates several params atomically from the saved values; two separate `setParam` calls could drop or double-count a moment. Reaching zero is handled by whichever tab notices first and guarded on `run` so it only fires once. The chime is synthesised (`components/sound.ts`) and needs a prior click to be allowed.
