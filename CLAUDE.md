...
- `package.json` exposes `mcp/bin.mjs` as a bin, so people using a hosted copy can run the bridge with `npx -y github:D3SPR/d3shboard --allow-origin <site>` without cloning. The wrapper checks the Node version before importing the TypeScript server.
