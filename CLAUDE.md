...
- The **designer** (`src/ui/ComponentMaker.tsx`) is the editor for every component: a mini canvas at the card's real shape with draggable, resizable pieces, per-piece settings, and the whole component's look beneath it. Per-piece `scale`, `tint` and `font` override what the component sets; leave them unset and the piece follows. `ensureSource` sets up a data source the moment a value is dropped in, so data never has to be arranged in advance.
